UPDATE mt_company set company_name = 'PJ Team Server' WHERE company_code = '';

select company_code, company_name, * from mt_company;

update global_config set property_value = 'N' where property_name = 'enableSendEmail';
update global_config set property_value = '' where property_name = 'mailpassword';
update global_config set property_value = 1 where property_name = 'mailport';
update global_config set property_value = 'smtp.office.com' where property_name = 'mailhost';

select * from global_config where property_name like '%mail%';

update mt_employee_glbl set email_address = null where employee_code = '';
select email_address from mt_employee_glbl;

update mt_user
set encrypted_private_key = '',
    public_key            = ''
where user_id = '';

select * from mt_user where alias like '%Larr%';
-------------------------------------------------------------------------------
select pg_terminate_backend(pid) from pg_stat_activity where datname = 'pjmain3' and client_addr != '172.18.0.100';

select * from sys_config where properties_name like '%projectQuotationAndOrderVersion%';

select * from sys_config where properties_name like '%displayFormDescription%';

select * from sys_config where properties_name like '%th6BCURL%';

select * from SYS_CONFIG where PROPERTIES_NAME = 'synergixCustomer';

select * from global_config where property_name = 'measurementDecimalPlaces';

select properties_value from sys_config where properties_name = 'th6BCURL';

select * from global_config where property_name = 'enableSendEmail';

select (select r.relname from pg_class r where r.oid = c.conrelid) as "table",
       (select array_agg(attname) from pg_attribute
        where attrelid = c.conrelid and ARRAY[attnum] <@ c.conkey) as col,
       (select r.relname from pg_class r where r.oid = c.confrelid) as ftable
from pg_constraint c
where c.confrelid = (select oid from pg_class where relname = 'mt_company') and
      c.confkey @> (select array_agg(attnum) from pg_attribute
                    where attname = 'company_code' and attrelid = c.confrelid);

select * from global_config where property_name = 'folderPathForAttachmentFiles';

UPDATE GLOBAL_CONFIG SET PROPERTY_VALUE = '' WHERE PROPERTY_NAME = 'folderPathForAttachmentFiles' and PROPERTY_VALUE = ' ';

select * from sys_config where properties_name = 'th6Host';